...
Creating a new syntax colouration template was at first a difficult task - in essence, it was identical to building a parser for Sketch. Luckily, CodeMirror provides a simple module definition plugin, which reduces the task down to the construction of a finite state machine with the help of regular expressions. The task was far easier to tackle this way, and although it lacks the fine-grained control of writing a custom parser the results are very convincingly. The state machine is as follows:
...
The page itself (Figure \ref{fig:ide-full}) is a very simple arrangement of div blocks, a canvas, and a textarea which is in turn replaced by CodeMirror in combination with page-wide CSS designed to match the CodeMirror theme which matched our design specification. [[REWORD ME]]

\begin{figure}[!h]
\centering
\includegraphics[width=0.8\textwidth]{images/sketch-ide}
